...
expected_sha="8b7ea35c71dccd3bb8c4f65b9356689651bb1bc945bc4c67e03e18307431487c"
expected_game_sha="b0e5ac9e7500071ea8ecec9692f9956e4ee7688ba7471942a469d2ef346ea67f"
gate_rva="0x206870"
...
probe_mode="${WRF_GATE_PROBE_MODE:-producer}"
...
if [[ -n "${WRF_GATE_PROBE_GDB_DATA:-}" ]]; then
    ...
    private_root="/home/owendb/.local/share/wrf-probe-tools/root3"
...
pid="${1:-}"
find_game_pid() {
    local maps process_dir candidate=""
    for maps in /proc/[0-9]*/maps; do
        process_dir="${maps%/maps}"
        [[ "$(stat -c %u "$process_dir" 2>/dev/null || true)" == "$(id -u)" ]] || continue
        if awk 'tolower($0) ~ /wrfrontiers-win64-shipping\.exe/ { found=1; exit } END { exit !found }' "$maps" 2>/dev/null; then
...
if [[ "$probe_mode" == "early-producer" || "$probe_mode" == "early-mmap" || "$probe_mode" == "early-acquisition" ]]; then
    dll_path="$(dirname -- "$game_path")/acclient64.dll"
    module_base="0x0"
else
...
export WRF_GATE_PROBE_MODE="$probe_mode"
export WRF_GATE_PROBE_HANDOFF_RVA="$handoff_rva"
